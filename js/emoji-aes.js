...
function encrypt(){
  ...
  var key = document.getElementById("key-for-encryption").value;
  var encrypted = CryptoJS.AES.encrypt(message.value, key).toString();
...
function decrypt(){
  ...
  var key = document.getElementById("key-for-decryption").value;
  ...
  var plaintext  = CryptoJS.AES.decrypt(unemojified, key).toString(CryptoJS.enc.Utf8);
